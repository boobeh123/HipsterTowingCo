...
describe('E2E Views Testing', () => {
  let app, testUser, testInspection, testContact;
  let authCookie;
  ...
  beforeEach(async () => {
    try {
      // Clear all collections with shorter timeout
      await Promise.race([
        Promise.all([
          User.deleteMany({}),
          Todo.deleteMany({}),
  ...
  }, 8000); // 8 second timeout for beforeEach
  ...
  describe('Form Submissions', () => {
    ...
    it('should handle inspection creation for authenticated user', async () => {
      ...
      expect(newInspection.userId.toString()).toBe(testUser._id.toString());
    });
