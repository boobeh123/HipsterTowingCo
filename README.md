...
## Version History 
# 🛠️ pretriq Patch 1.6.021
📅 **Release Date:** September 20th, 2026

## 📢 Developer's Notes - 

- Add favicon fallback & site.webmanifest
---------------------------------------------------------------------------------------------------------------------------
...
- Updated several packages with `npm audit fix`
  - Updated Node to v22 - required for updating packages from `npm audit fix`
- Added middleware for POST requests on /profile/photo 
- Added validation which prevent empty images from being submitted
- Added file size limit & extension+MIME checks to multer
- Updated errorHandler 
- Updated EJS tag so unescaped data doesn't display
- The navigation's active state now displays 
  - Being on /dashboard changes dashboards state in the nav
- `res.redirect('back')` from Express 5 sent users to an unexisting route during password recovery
  - When your passwords did not match while typing in a new password & confirm password, it would send you to `/reset/back/` instead of `/reset/:token/`
- Updated tests suites & remove unused tests
- Relocate a few client-side js functions from `public/js/main.js` to `public/js/utils.js`
  - Functions are global by the time the handlers from `main.js` invoke the utils
- Add `CLAUDE.md` to the project repository
  - Welcome Claude & my intro to Claude Code
- Add a compound index to the `InspectionSchema`
---------------------------------------------------------------------------------------------------------------------------
# 🛠️ pretriq Patch 1.6.019
📅 **Release Date:** July 9th, 2026

## 📢 Developer's Notes - Cookie banner

- I did not give users the option to decide if they want Google Analytics to track
  - Saw this & got properly spooked
  - ![spooks](/imagesgit/spooks.png)
---------------------------------------------------------------------------------------------------------------------------
